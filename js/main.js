...
const backetCount = document.querySelector('.backet-count')
...
function backetLength() {
    backetCount.innerHTML = dataBacket.length
    if (dataBacket.length === 0) {
      backetCount.style.display = "none"
      backetBtn.style.justifyContent = "center"
    }else{
      backetCount.style.display = "block"
      backetBtn.style.justifyContent = "space-around"
    }
}
  ...
  if (inputName.value.trim().length === 0) {
    ...
    if (inputNumber.value.trim().length < 14) {
      ...
      if (inputAddress.value.trim().length < 10) {
        ...
        inputAddress.classList.add("input-danger");
      } else {
        inputAddress.classList.remove("input-danger");
        createMessage();
      }
    }
  }
...
function createMessage() {
  ...
  message += "Адрес: " + inputAddress.value + "%0A";
  message += "Описание: " + textarea.value + "%0A%0A%0A";


  dataBacket.forEach((item, i) => {
    if (item.totalCost !== 0) {
      message += `${item.title} - ${item.count}шт.  цена: ${item.totalCost}%0A`;
    }
  });
